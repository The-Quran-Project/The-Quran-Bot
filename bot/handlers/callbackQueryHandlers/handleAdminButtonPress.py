...
async def handleAdminButtonPress(u: Update, c):
    ...
    if data[1] == "users":
        if data[2] == "len":
            await query.answer(f"There are {len(db.getAllUsers())} users", show_alert=True)
        elif data[2] == "all":
            ...
            await query.message.reply_document(file, quote=True, caption=f"Total Users: {len(allUsers)}")

    elif data[1] == "chats":
        if data[2] == "len":
            await query.answer(f"There are {len(db.getAllChat())} chats", show_alert=True)
        elif data[2] == "all":
            await query.answer("Sending all chats")
            allChats = db.getAllChat()
            file = BytesIO(json.dumps(allChats).encode())
            file.name = "chats.json"
            await query.message.reply_document(file, quote=True, caption=f"Total Chats: {len(allChats)}")
        ...
        if data[2] == "len":
            ...
            await query.message.reply_document(file, quote=True, caption=f"Total Admins: {len(allAdmins)}")
