...
class CachedCollection:
    ...
    def _load_initial_data(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(documents)} documents from {self.collection.name}"
            )
            return True
    ...
    def insert_one(self, document: Dict) -> None:
        ...
        self.sync_manager.queue_operation(self.collection.name, "insert_one", document)
    ...
    def update_one(
        ...
        self.sync_manager.queue_operation(
            self.collection.name, "update_one", filter_dict, update_dict, upsert
        )
    ...
    def delete_one(self, filter_dict: Dict) -> None:
        ...
        # Queue operation for MongoDB
        self.sync_manager.queue_operation(
            self.collection.name, "delete_one", filter_dict
        )
        ...
        while not self.operations_queue.empty():
            try:
                ...
                if op_type == "insert_one":
                    ...
                    filter_dict, update_dict, upsert = (
                        args[0],
                        args[1],
                        args[2] if len(args) > 2 else False,
                    )
                    self.bulk_ops[collection_name].append(
                        UpdateOne(filter_dict, update_dict, upsert=upsert)
                    )
    ...
    def _execute_bulk_ops(self):
        ...
        for collection_name, ops in self.bulk_ops.items():
            ...
            try:
                ...
                logger.info(
                    f"Synced {len(ops)} operations to {collection_name} in {(end_time - start_time) * 1000:.2f}ms"
                )
            except PyMongoError as e:
                logger.error(
                    f"Error executing bulk operations on {collection_name}: {e}"
                )
...
class CachedDatabase:
    ...
    def update_user_settings(self, user_id: int, settings: Dict) -> None:
        ...
        self.users.update_one(
            {"_id": user_id}, {"$set": {"settings": updated_settings}}
        )
    ...
    def update_chat_settings(self, chat_id: int, settings: Dict) -> None:
        ...
        if "restrictedLangs" in updated_settings:
            updated_settings["restrictedLangs"] = list(
                set(updated_settings["restrictedLangs"])
            )

        self.chats.update_one(
            {"_id": chat_id}, {"$set": {"settings": updated_settings}}
        )
    ...
    def update_channel(self, channel_id: int, settings: Dict) -> None:
        ...
        self.channels.update_one(
            {"_id": channel_id}, {"$set": {"settings": updated_settings}}
        )
    ...
    def update_active_users(self, user_id: int) -> None:
        ...
        self.activeUsers.update_one(
            {"_id": "users"}, {"$addToSet": {"list": user_id}}, upsert=True
        )
    ...
    def update_counter(self) -> None:
        ...
        self.analytics.update_one({"_id": date}, {"$inc": {"requests": 1}}, upsert=True)
...
async def comprehensive_example():
    ...
    # Update user settings
    db.update_user_settings(user_id, {"primary": "en", "secondary": "ar", "font": 2})
    ...
    # Update channel settings
    db.update_channel(channel_id, {"language": "en", "auto_post": True})
